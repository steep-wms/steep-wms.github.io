...
* You can use this endpoint to cancel the execution of a process chain (see
  example below).
* The priority can only be modified if the <ScrollLink href="#process-chain-status">process chain status</ScrollLink>
  is `REGISTERED` or `RUNNING`. Otherwise, the operation will result in HTTP error 422.
...
| 422          | The process chain's priority could not be modified because the process chain is already finished, i.e. the <ScrollLink href="#process-chain-status">process chain status</ScrollLink> is not `REGISTERED` or `RUNNING` |
