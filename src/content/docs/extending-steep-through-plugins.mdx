...
| scriptFile<br/>*(required)* | string | The path to the plugin's Kotlin script file. The file should have the extension `.kts`. The path is relative to Steep's application directory, so a valid example is `conf/`<wbr />`plugins/`<wbr />`fileOrEmptyList.kts`.
