...
<div className="grid grid-cols-1 lg:grid-cols-2 gap-x-8 gap-y-4 lg:gap-y-8 not-prose my-8 items-start">
  ...
  <div className="flex flex-row items-center max-w-lg">
    ...
    <div className="flex w-fit items-center">
      <ChevronRight size="2rem" className="m-0" />
    </div>
    <Tiles />
  </div>
...
The actions in the second workflow step are intended to run in parallel. Be default, each Steep instance spawns only one agent, so the actions will be executed sequentially. If you want to execute this workflow as it is intended, you need to enable <DocsLink href="#parallelization">parallelization</DocsLink>.

<Admonition>
Note that each instance of the segmentation service requires up 6 GB of RAM. If you want to run the workflow on a single machine only, limit the number of agents so that you don't exceed your main memory.
</Admonition>
