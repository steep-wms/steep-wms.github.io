...
Semantic segmentation is the process of assigning an object class (e.g. *tree*, *building*, or *street*) to each pixel of an image. Our workflow creates segmentation masks, which are images where the detected classes is represented by different colors.
...
<div className="grid grid-cols-1 lg:grid-cols-2 gap-x-8 gap-y-4 lg:gap-y-8 not-prose my-8 items-start">
  ...
  <div className="flex flex-row items-center max-w-lg">
    <div className="w-72">
      <Tiles n={1} src={`${process.env.basePath}/images/home/tiles.jpg`} gap={false} imageSize={650} />
    </div>
    <div className="flex w-16 items-center">
      <ChevronRight height="100%" width="100%" />
    </div>
    <div className="w-72">
      <Tiles n={2} src={`${process.env.basePath}/images/home/tiles.jpg`} imageSize={650} />
    </div>
  </div>

  <div className="mt-4 lg:mt-0">
    <div className="text-lg font-normal mb-2">Apply image segmentation</div>
    <p>The <em>segmentation service</em> is applied in a for-each action to each image tile to create the segmentation masks.</p>
  </div>
