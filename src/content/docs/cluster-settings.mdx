...
> The interval at which Steep's main thread looks for orphaned entries in its internal remote agent registry (specified as a <ScrollLink href="#durations">duration</ScrollLink>). Such entries may (very rarely) happen if there is a network failure during deregistration of an agent. You normally do not have to change this configuration.
