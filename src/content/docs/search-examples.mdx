...
> Search for workflows or process chains that have started or finished on <Fragment suppressHydrationWarning={true}>{dayjs().format("DD MMMM YYYY")}</Fragment>.

<code suppressHydrationWarning={true}>&lt;{dayjs().format("YYYY-MM-DDTHH:mm")}</code>

> Search for workflows or process chains that have started or finished before <Fragment suppressHydrationWarning={true}>{dayjs().format("HH:mm")}</Fragment> on <Fragment suppressHydrationWarning={true}>{dayjs().format("DD MMMM YYYY")}</Fragment>.

<code suppressHydrationWarning={true}>&gt;={dayjs().format("YYYY-MM-DDTHH:mm:ss")}</code>

> Search for workflows or process chains that have started or finished at or after <Fragment suppressHydrationWarning={true}>{dayjs().format("HH:mm:ss")}</Fragment> on <Fragment suppressHydrationWarning={true}>{dayjs().format("DD MMMM YYYY")}</Fragment>.

<code suppressHydrationWarning={true}>start:{dayjs().subtract(1, "day").format("YYYY-MM-DD")}..{dayjs().format("YYYY-MM-DD")}</code>

> Search for workflows or process chains that have been started between <Fragment suppressHydrationWarning={true}>{dayjs().subtract(1, "day").format("DD MMMM YYYY")}</Fragment> and <Fragment suppressHydrationWarning={true}>{dayjs().format("DD MMMM YYYY")}</Fragment> (inclusive).
