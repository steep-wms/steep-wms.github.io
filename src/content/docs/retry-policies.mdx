...
the <ScrollLink href="#service-metadata">service metadata</ScrollLink> or
per <ScrollLink href="#execute-actions">executable action</ScrollLink> in the
workflow.
...
| delay<br/>*(optional)*              | <ScrollLink href="#durations">duration</ScrollLink> | The amount of time that should pass between two attempts. The default is `0`, which means the operation will be retried immediately.
| exponentialBackoff<br/>*(optional)* | number   | A factor for an exponential backoff (see description below)
| maxDelay<br/>*(optional)*           | <ScrollLink href="#durations">duration</ScrollLink> | The maximum amount of time that should pass between two attempts. Only applies if `exponentialBackoff` is larger than `1`. By default, there is no upper limit.
