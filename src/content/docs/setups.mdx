...
| sshUsername<br/>*(optional)*           | string  | An optional username for the SSH connection to the created VM. Overrides the <ScrollLink href="#steepyaml">main configuration</ScrollLink> item `steep.`<wbr />`cloud.`<wbr />`ssh.`<wbr />`username` if it is defined.
