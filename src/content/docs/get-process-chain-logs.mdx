...
| runNumber<br />*(optional)*  | Specifies the actual run of the process chain whose log file to return (see the section on <ScrollLink href="#process-chains">process chains</ScrollLink> for details on runs). If no run number is given, the endpoint will return the log file of the latest run. Note that a process chain that currently has the <ScrollLink href="#process-chain-status">status</ScrollLink> `PAUSED` or `REGISTERED`, does not have a latest run. In this case, the endpoint will return HTTP status code 404.
