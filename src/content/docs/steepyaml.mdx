The file `steep.yaml` contains the main configuration of Steep. In this section,
...
`steep.`<wbr />`cluster.`<wbr />`eventBus.`<wbr />`publicPort` becomes `STEEP_`<wbr />`CLUSTER_`<wbr />`EVENTBUS_`<wbr />`PUBLICPORT`.
You may use YAML syntax to specify environment variable values. For example,
the array `steep.`<wbr />`agent.`<wbr />`capabilities` can be specified as follows:
